...
class FacePreprocessor:
    ...
    Solo manipula imágenes en formato NumPy/OpenCV.
    """
    ...
    def resize_face(self, face: np.ndarray) -> np.ndarray:
        return cv2.resize(
            face,
            self.target_size,
    ...
    def prepare_face(
        ...
        """Pipeline completo: expand → crop → resize → normalize."""
        bbox = self.expand_bbox(bbox, frame.shape)
        face = self.crop_face(frame, bbox)

        if face.size == 0:
            raise ValueError("Face crop vacío tras el recorte.")
